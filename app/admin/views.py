...
def comment_del(id=None):
    ...
    movie = Movie.query.get_or_404(int(comment.movie_id))
    db.session.delete(comment)
    db.session.commit()
    flash(" 删除评论成功！", "ok")
    reason = "删除用户:%s 对电影：%s评论" % (user.name, movie.title)
    add_oplog(reason)
...
def moviecol_del(id=None):
    ...
    movie = Movie.query.get_or_404(moviecol.movie_id)
    db.session.delete(moviecol)
    db.session.commit()
    flash(" 删除收藏成功！", "ok")
    reason = "删除用户：%s 对电影： %s的收藏" % (user.name, movie.title)
    add_oplog(reason)
...
def role_add():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("添加角色成功", "ok")
        reason = "添加角色%s, 权限：%s " % (role.name, role.auths)
        add_oplog(reason)
...
def role_edit(id=None):
    ...
    if request.method == "GET":
        form.auths.data = list(map(lambda v: int(v), old_role_auths.split(",")))
    if form.validate_on_submit():
        data = form.data
